...
var ImportScriptFile = () => {
  return test();
  fetch("https://diegofrr.github.io/vlibras-widget-plus/app2/vlibras-plugin.js")
    .then((response) => response.text())
    .then((code) => eval(code))
    .then(
      () =>
        new window.VLibras.Widget(
          "https://diegofrr.github.io/vlibras-widget-plus/app2"
        )
    );
};
  ...
  window.VLibras = (function (t) {
    ...
    function (t, e, n) {
      ...
      const i = () => v(document.body, "vpw-fullscreen"),
        ...
        m = (t) => {
          h(u(".vpw-skip-welcome-message"), "vp-disabled", t),
            h(u("[vp-click-blocker]"), "vp-enabled", t);
        },
        y = () =>
          v(u("[vp-controls]"), "vpw-controls") &&
          !v(u(".vp-guide-container"), "vp-enabled") &&
          v(u("[vw-plugin-wrapper]"), "active"),
        x = (t) => {
          t
        ...
        L = (t = !0) => {
          t
            ? u("[vp-controls] .vpw-controls-button").setAttribute(
                "disabled",
                !0
              )
            : u("[vp-controls] .vpw-controls-button").removeAttribute(
                "disabled"
              );
        },
        T = (t, e, n) => {
    ...
    function (t, e, n) {
      ...
      const {
        ...
        { ROOT_PATH: d } = n(3),
        p = ["TL", "T", "TR", "L", "R", "BL", "B", "BR"],
        v = ["icaro", "hosana", "guga", "random"];
      t.exports = function (...t) {
        ...
        let f = e ? e.rootPath : t[0],
          ...
          g = e ? e.opacity : t[2],
          h = e.avatar;
        void 0 === f ? (f = d) : f && !f.endsWith("/") && (f += "/"),
          (isNaN(g) || g < 0 || g > 1) && (g = 1),
          p.includes(w) || (w = "R"),
          v.includes(h) || (h = "icaro");
        const b = new o(),
          m = new i({
            rootPath: f,
            pluginWrapper: b,
        ...
        let y;
        window.onload && (y = window.onload),
          (window.onload = () => {
            a("[vw]").forEach((t) => {
              r("[vp]"),
                t ||
                  (t.removeAttribute("vw"),
                  location.hostname.includes("correios.com.br") &&
                    l(t, "enabled"));
            }),
              y && y(),
              (this.element = r("[vw-plugin-wrapper]").closest("[vw]"));
            const t = r("[vw-plugin-wrapper]"),
              e = r("[vw-access-button]");
            m.load(r("[vw-access-button]"), this.element),
              b.load(r("[vw-plugin-wrapper]")),
              window.addEventListener("vp-widget-wrapper-set-side", (t) => {
                const e = t.detail;
                if (!e || !p.includes(e)) return;
                (this.element = u()),
                  (this.element.style.left = e.includes("L")
                    ? "0"
                    : ["T", "B"].includes(e)
                    ? "50%"
                    : "initial"),
                  (this.element.style.right = e.includes("R")
                    ? "0"
                    : "initial"),
                  (this.element.style.top = e.includes("T")
                    ? "0"
                    : ["L", "R"].includes(e)
                    ? "50%"
                    : "initial"),
                  (this.element.style.bottom = e.includes("B")
                    ? "0"
                    : "initial"),
                  (this.element.style.transform = ["L", "R"].includes(e)
                    ? "translateY(calc(-50% - 10px))"
                    : ["T", "B"].includes(e)
                    ? "translateX(calc(-50% - 10px))"
                    : "initial");
                const n = r("[vw-access-button]");
                n.classList.toggle("isLeft", e.includes("L")),
                  n.classList.toggle("isTopOrBottom", "TB".includes(e)),
                  window.plugin && (window.plugin.position = e);
              }),
              ...
              window.addEventListener("vp-widget-close", (n) => {
                e.classList.toggle("active"),
                  ...
                  s(r("div[vp-change-avatar]"), "active"),
                  s(r("div[vp-additional-options]"), "vp-enabled"),
                  l(r("div[vp-controls]"), "vpw-selectText"),
                  c(!1);
              }),
        ...
        (this.element = t),
          ...
          this.element.addEventListener("click", async () => {
            this.element.classList.toggle("active"),
              ...
              this.ready && r(!0);
            const { Plugin: t } = await n.e(0).then(n.bind(null, 17)),
              { loadTextCaptureScript: e } = await n
                .e(0)
                .then(n.bind(null, 18));
