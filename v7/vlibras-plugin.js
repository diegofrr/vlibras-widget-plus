...
const initBtn = shadow.querySelector("#vlibras-access-button"),
  open = () => {
    if (widget) return (widget.dataset.active = !0);
    const t = document.createElement("script");
    ((t.src = `${path}/vlibras-plugin-app.umd.cjs`),
      (t.async = !0),
      (t.onload = () => {
        ((widget = document.getElementById("vlibras-app-root")),
          widget && (widget.dataset.active = !0));
      }),
...
((initBtn.onclick = open),
  ...
  localStorage.getItem("@vlibras-wp")?.includes('"isActive":true') && open());
